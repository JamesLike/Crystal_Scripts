! CALCULATES DIFFERENCE STRUCTURE FACTORS AND WEIGHTS.
! SORT ORDER IN OUTPUT IS THE SAME AS IN FILE1
! CALC. MEAN SQUARE (AND NOT SQUARED MEAN) 
! INPUT: FILE1 REFERENCE: NORMALLY LIGHT (FOR LIGHT MINUS DARK AMPLITUDES)
!                         COLUMNS H,K,L,F,SIGF REQUIRED, 6th COLUMN IGNORED 
!        FILE2          : IF HKL MATCH DIFFERENCE WILL BE WRITTEN OUT
!                         (NORMALLY DARK FOR LIGHT MINUS DARK)
!                         COLUNS H,K,L,F,SIGF REQUIRED, 6th COLUMN IGNORED 
!        FILE3          : PHASES MUST BE IN 6th COLUMN, WILL BE ONLY TRANSFERED TO
!                         OUTPUT FOR IMMEDIATE FFT
!        OUTFIL         : OUTPUT FILE NAME
!
! WEIGHT USED BY THIS PROGRAM:
! 
!                       1
!  W = ----------------------------------- 
!               F**2         SIG(F)**2
...
!             < F**2 >     < SIG(F)**2 >
! 
! NOTE ! OTHERS USES THE SQUARE OF THE MEAN OF THE ABSOLUTE VALUES (E.G. <|F|>**2)
!        WHEREAS THIS PROGRAM USES THE MEAN OF SQUARES (OR THE MEAN SQUARE
!        WHICH IS MORE CORRECT IN A STATISTICAL MANNER) 
! STRUCTURE FACTORS WILL BE NORMALIZED BY THE MEAN WEIGHT IN ORDER TO PRESERVE
! ABSOLUTE SCALE OF A WEIGHTED MAP. ELECTRON DENSITY OF UNWEIGHTED MAPS CALCULATED 
! BY THE OUTPUT OF THIS PROGRAM HAS TO BE DIVIDED BY THE MEAN WEIGHT TO BE ON 
! ABSOLUTE SCALE
! BUG WITH OUTPUT DATA REMOVED
! MARIUS SCHMIDT SEPT/OCT 2000

CHARACTER*60 FIL1, FIL2, FIL3, OFIL
DIMENSION FDARK(-250:250, -250:250, -250:250, 3)
!
! INITIALIZE FDARKSs PHASE FIELD (PHASE MUST BE DEFINED BY SFCALC PROGRAM
! PHASE ANGLE 0 ALLOWED THEREFORE INITALIZE WITH 999.9 
DO 50 I = -150, 150
    DO 50 J = -150, 150
        DO 50 K = -150, 150
...
GOTO 1300
1400      CONTINUE
! DEBUG    write(*,*) IP,IHMIN,IHMAX,KMIN,KMAX,LMIN,LMAX
! 
I2 = 0
...
! LOOP OVER AMPLITUDES IN FILE 1 AND SEARCH MATCHING HKLs IN FILE 2.
! THEN CALCULATE MEAN DIFFERENCE AMPLITUDE AND THE MEAN OF THE
! SUM OF SIGMA1 AND SIGMA2.
I1 = 0
IC = 0
DFSQ = 0.0
S12SQ = 0.0
...
GOTO 2700
2900      CONTINUE
!           write(*,*)IO,WMEAN,WZMEAN
WMEAN = WMEAN / IO
WZMEAN = WZMEAN / IO
! 
OPEN(UNIT = 12, FILE = OFIL)
! LOOP AGAIN OVER ALL HKLs in FILE1 (PRESERVES SORT ORDER !!!!!), 
...
IF (FDARK(IH, K, L, 1).NE.0.0.AND.FDARK(IH, K, L, 3).NE.999.9) THEN
    DF = (F1 - FDARK(IH, K, L, 1))
    DFDW = DFDW + DF / WMEAN
    ! DON'T TAKE SQRT, WILL BE SQUARED IN NEXT LINE AGAIN
    S12 = SIGF1**2 + FDARK(IH, K, L, 2)**2
    ! MARIUS WEIGHT MEAN SQUARE
    W = 1 / (1 + (DF**2 / DFSQM) + (S12 / S12SQM))
    ! WEIGHT SQUARED MEAN
